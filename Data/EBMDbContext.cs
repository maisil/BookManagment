...
namespace BookManagment.Data
{
    public class EBMDbContext:DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
           ...
           modelBuilder.Entity<BookGenre>().HasOne(bg=>bg.Book).WithMany(bg=>bg.Genres).HasForeignKey(bg=>bg.GenreId);
           modelBuilder.Entity<BookGenre>().HasOne(bg => bg.Genre).WithMany(bg => bg.Books).HasForeignKey(bg => bg.BookId);
